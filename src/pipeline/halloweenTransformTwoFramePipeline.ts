...
export async function runHalloweenTransformTwoFramePipeline(
  ...
  const selectedStyle = 'halloweenTransformTwoFrame';
    ...
    while (attempt < maxAttempts && !finished) {
      ...
      try {
        if (options.emitLog && options.requestId) {
          options.emitLog(`🎃 Generating Halloween Transform Two Frame song with ${selectedStyle} style... (Attempt ${attempt})`, options.requestId);
        }
        ...
        const imageJson: string | Record<string, any> | null = await executePipelineStepWithTracking({
          stepName: 'HALLOWEEN TRANSFORM TWO FRAME IMAGE PROMPTS',
        ...
        if (imageJson) {
          const parsed = typeof imageJson === 'string' ? safeJsonParse(imageJson, 'HALLOWEEN TRANSFORM TWO FRAME IMAGE PROMPTS') : imageJson;
        ...
        try {
          ...
          halloweenTransformTwoFrameLogVideoPrompt(imagePromptsJson);
          videoJson = await executePipelineStepWithTracking({
            stepName: 'HALLOWEEN TRANSFORM TWO FRAME VIDEO PROMPTS',
            promptTemplate: halloweenTransformTwoFrameVideoPrompt,
            options: { model: videoModel, temperature: videoTemperature },
            params: { image_prompts: imagePromptsJson },
            requests
          });
          if (videoJson) {
            const parsed = typeof videoJson === 'string' ? safeJsonParse(videoJson, 'HALLOWEEN TRANSFORM TWO FRAME VIDEO PROMPTS') : videoJson;
            if (options.emitLog && options.requestId) {
        ...
        for (let segmentIndex = 0; segmentIndex < videoSegments.length; segmentIndex++) {
          ...
          try {
            halloweenTransformTwoFrameLogTitlePrompt(segmentLines, segmentVideoPrompts, globalStyle);
            titleJson = await executePipelineStepWithTracking({
              stepName: 'HALLOWEEN TRANSFORM TWO FRAME TITLE',
              promptTemplate: halloweenTransformTwoFrameTitlePrompt,
            ...
            if (titleJson) {
              const parsed = typeof titleJson === 'string' ? safeJsonParse(titleJson, 'HALLOWEEN TRANSFORM TWO FRAME TITLE') : titleJson;
          ...
          try {
            ...
            for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
              ...
              while (imageAttempts < maxImageAttempts && !groupImagePrompt) {
                ...
                logHalloweenTransformTwoFrameGroupImagePrompt(globalStyle, threePrompts);
                const groupImageJson: string | Record<string, any> | null = await executePipelineStepWithTracking({
                  stepName: 'HALLOWEEN TRANSFORM TWO FRAME GROUP IMAGE',
                  promptTemplate: halloweenTransformTwoFrameGroupImagePrompt,
                  options: { model: groupImageModel, temperature: groupImageTemperature },
                  params: { globalStyle: globalStyle, prompts: threePrompts },
                  requests
                });
                if (groupImageJson) {
                  const parsed = typeof groupImageJson === 'string' ? safeJsonParse(groupImageJson, 'HALLOWEEN TRANSFORM TWO FRAME GROUP IMAGE') : groupImageJson;
              ...
              while (videoAttempts < maxVideoAttempts && !groupVideoPrompt) {
                ...
                logHalloweenTransformTwoFrameGroupVideoPrompt(groupImagePrompt);
                const groupVideoJson: string | Record<string, any> | null = await executePipelineStepWithTracking({
                  stepName: 'HALLOWEEN TRANSFORM TWO FRAME GROUP VIDEO',
                  promptTemplate: halloweenTransformTwoFrameGroupVideoPrompt,
                  options: { model: groupVideoModel, temperature: groupVideoTemperature },
                  params: { groupImagePrompt: groupImagePrompt },
                  requests
                });
                if (groupVideoJson) {
                  const parsed = typeof groupVideoJson === 'string' ? safeJsonParse(groupVideoJson, 'HALLOWEEN TRANSFORM TWO FRAME GROUP VIDEO') : groupVideoJson;
