...
const actualPath = path.join(__dirname, 'hashtagsPrompt.prompt.txt');
const templatePath = path.join(__dirname, 'hashtagsPrompt.prompt.template.txt');

if (fs.existsSync(actualPath)) {
    hashtagsPromptTemplate = fs.readFileSync(actualPath, 'utf-8');
} else {
    hashtagsPromptTemplate = fs.readFileSync(templatePath, 'utf-8');
    console.warn('⚠️  Using template prompt for shortStudyHashtagsPrompt. Copy .template.txt to .txt for production use.');
}
